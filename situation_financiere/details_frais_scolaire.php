...
// Filtre par année scolaire et recherche
$selectedAnnee = trim($_GET['anneeScolaire'] ?? '');
$search = trim($_GET['q'] ?? '');

// Années scolaires disponibles
$anneesStmt = $pdo->query("SELECT DISTINCT anneeScolaire FROM paiement WHERE anneeScolaire IS NOT NULL AND anneeScolaire != '' ORDER BY anneeScolaire DESC");
...
if ($search !== '') {
    $where[] = "(menage LIKE :q OR observation LIKE :q)";
    $params[':q'] = "%$search%";
}
...
// Synthèse rapide
$stmtTotal = $pdo->prepare("SELECT COALESCE(SUM(montantAPayer),0) as attendu, COALESCE(SUM(montantPayer),0) as percu, COALESCE(SUM(resteAPayer),0) as reste FROM paiement WHERE $whereSql");
$stmtTotal->execute($params);
$totaux = $stmtTotal->fetch(PDO::FETCH_ASSOC);

// Liste des paiements
$stmtList = $pdo->prepare("SELECT * FROM paiement WHERE $whereSql ORDER BY id DESC");
...
<div class="container-fluid my-4">
    ...
    <div class="d-flex justify-content-between align-items-center p-3 mb-4 bg-light border rounded no-print">
        <div>
            ...
            <h4 class="fw-bold text-primary d-inline-block m-0">🏫 Liste Détaillée - Frais Scolaires</h4>
        </div>
        <form method="GET" class="d-flex align-items-center gap-2">
            <input type="text" name="q" value="<?= htmlspecialchars($search) ?>" class="form-select form-select-sm" placeholder="Recherche ménage...">
                ...
                <table class="table table-hover table-striped align-middle mb-0">
                    ...
                    <tbody>
                        <?php if (empty($paiements)): ?>
                            <tr><td colspan="8" class="text-center text-muted py-4">Aucun enregistrement trouvé.</td></tr>
                        <?php else: ?>
                            <?php foreach ($paiements as $p): ?>
                                <tr>
                                    ...
                                    <td class="small text-muted"><?= htmlspecialchars($p['observation'] ?? '-') ?></td>
