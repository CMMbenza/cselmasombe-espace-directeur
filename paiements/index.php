...
// Filtres
$q       = trim((string)($_GET['q'] ?? '')); // texte: nom ménage / tel / obs
$annee   = trim((string)($_GET['annee_scolaire'] ?? ''));
$from    = trim((string)($_GET['from'] ?? '')); // dateCreated >= from
$to      = trim((string)($_GET['to'] ?? ''));   // dateCreated <= to
...
try {
  // Années distinctes depuis paiement
  $stmtA = $pdo->query("SELECT DISTINCT anneeScolaire FROM paiement WHERE anneeScolaire IS NOT NULL AND anneeScolaire<>'' ORDER BY anneeScolaire DESC");
  ...
  if ($q !== '') {
    $wheres[] = "(m.noms LIKE :like OR m.telephone LIKE :like OR p.observation LIKE :like)";
    $params[':like'] = "%{$q}%";
  }
  if ($annee !== '') {
    $wheres[] = "p.anneeScolaire = :annee";
    $params[':annee'] = $annee;
  }
  if ($from !== '') {
    $wheres[] = "p.dateCreated >= :from";
    $params[':from'] = $from;
  }
  if ($to !== '') {
    $wheres[] = "p.dateCreated <= :to";
    $params[':to'] = $to;
  }
...
<div class="container">
  <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
    <h1 class="h4 mb-0">Paiements (scolarité)</h1>

    <form class="row g-2" method="get">
      <div class="col-12 col-md-4">
        <input class="form-control form-control-sm" type="search" name="q" placeholder="Ménage / Téléphone / Observation" value="<?= e($q) ?>">
      </div>
      ...
      <div class="col-12 col-md-auto d-flex gap-2">
        ...
        <?php if ($q!=='' || $annee!=='' || $from!=='' || $to!==''): ?>
          <a class="btn btn-sm btn-outline-secondary" href="<?= BASE_URL ?>/paiements/index.php">Réinitialiser</a>
        <?php endif; ?>
      </div>
    </form>
  </div>
  ...
  <div class="card shadow-sm">
    <div class="card-body table-responsive">
      <table class="table table-sm align-middle">
        ...
        <tbody>
        <?php if (!$rows): ?>
          <tr><td colspan="10"><em>Aucun paiement trouvé.</em></td></tr>
